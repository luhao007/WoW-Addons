...
from toc import TOC
from utils import TOCS, get_libraries_list
...
class CheckManagedAddOns(unittest.TestCase):
    ...
    def test_check_addon_toc(self):
        ...
        for addon in os.listdir("AddOns"):
            if addon in ignored or "Test" in addon or "Blizzard_" in addon:
                continue

            filenames = [f"{addon}{toc}" for toc in TOCS]
            root = Path("AddOns") / addon
            self.assertTrue(
                any(os.path.exists(root / f"{f}") for f in filenames),
