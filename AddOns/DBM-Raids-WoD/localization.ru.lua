...
L:SetOptionLocalization({
	ChainsBehavior	= "Режим предупреждения для Цепи Скверны",
...
L:SetOptionLocalization({
	...
	MarkBehavior			= "Установить режим крика «Клеймо Легиона» для рейда (требуется лидер рейда)",
	...
	overrideMarkOfLegion	= "Не позволяйте лидеру рейда переопределять режим «Клеймо Легиона» (рекомендуется только для экспертов, которые уверены, что их собственные настройки не противоречат намерениям лидеров рейда)"
})
