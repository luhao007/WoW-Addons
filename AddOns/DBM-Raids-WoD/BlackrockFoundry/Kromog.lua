local mod	= DBM:NewMod(1162, "DBM-Raids-WoD", 2, 457)
...
local specWarnGraspingEarth			= mod:NewSpecialWarningMoveTo(157060, nil, DBM_CORE_L.AUTO_SPEC_WARN_OPTIONS.spell:format(157060), nil, nil, 12, nil, nil, "safenow")
local specWarnThunderingBlows		= mod:NewSpecialWarningSpell(157054, nil, nil, nil, 3)
local specWarnRipplingSmash			= mod:NewSpecialWarningDodge(157592, nil, nil, nil, 2, 2, nil, nil, "shockwave")
local specWarnStoneBreath			= mod:NewSpecialWarningCount(156852, nil, nil, nil, 2, 2, nil, nil, "breathsoon")
local specWarnSlam					= mod:NewSpecialWarningDefensive(156704, "Tank", nil, nil, 1, 2, nil, nil, "defensive")
...
local specWarnCalloftheMountain		= mod:NewSpecialWarningCount(158217, nil, nil, nil, 3, 2, nil, nil, "findshelter")
...
local timerThunderingBlowsCD		= mod:NewNextTimer(12, 157054, nil, nil, nil, 2, nil, DBM_COMMON_L.DEADLY_ICON, nil, 1, 4)
...
local timerStoneBreathCD			= mod:NewCDCountTimer(22, 156852, nil, nil, nil, 2, nil, DBM_COMMON_L.HEALER_ICON)
local timerSlamCD					= mod:NewCDTimer(23, 156704, nil, "Tank", nil, 5, nil, DBM_COMMON_L.TANK_ICON)
local timerWarpedArmorCD			= mod:NewCDTimer(14, 156766, nil, "Tank", nil, 5, nil, DBM_COMMON_L.TANK_ICON)
...
local timerCalloftheMountain		= mod:NewCastTimer(5, 158217, nil, nil, nil, 2, nil, DBM_COMMON_L.DEADLY_ICON)
...
function mod:SPELL_CAST_START(args)
	...
	if spellId == 157060 then
		...
		specWarnGraspingEarth:Play("getinyellowrunes")
		if self:IsMythic() then
