...
function ns:RegisterAddonHook(addon, callback)
    if C_AddOns.IsAddOnLoaded("TinyInspect-Reforged") then
        callback()
    else
    ...
    if (Bagnon and Bagnon.Item) then
        ...
        Module:AddUpdater(function(self)
            ...
            if (self.hasItem) then
                ...
                if (not equip) then
	            _,_,_,equip = C_Item.GetItemInfoInstant(self.info.hyperlink)
  	        end
	        ...
	        if (isgear) then
	            if (not level or not self.info.link) then
		    ...
		    if (self.info.link) then
  		         _, _, quality, _, _, class, subclass, _, equipSlot = C_Item.GetItemInfo(self.info.link)
                    end
