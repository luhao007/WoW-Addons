...
local function onExecute(self)
    if (self.dataType == "item") then
        local _, itemLink, quality, _, _, _, _, _, _, texture = C_Item.GetItemInfo(self.data)
        if (texture) then
...
local function ShowGemAndEnchant(frame, ItemLink, anchorFrame, itemframe)
    ...
    for i, v in ipairs(info) do
        ...
        if (v.link) then
            _, _, quality, _, _, _, _, _, _, texture = C_Item.GetItemInfo(v.link)
    ...
    if (enchantItemID) then
        num = num + 1
        icon = GetIconFrame(frame)
        _, ItemLink, quality, _, _, _, _, _, _, texture = C_Item.GetItemInfo(enchantItemID)
    ...
    elseif (enchantSpellID) then
        num = num + 1
        icon = GetIconFrame(frame)
        _, _, texture = C_Item.GetSpellInfo(enchantSpellID)
