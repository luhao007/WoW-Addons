...
function AuctionCommodityBuyConfirmationDialog.__private:_ActionHandler(manager, state, action, ...)
	...
	elseif action == "ACTION_PREPARE" then
		...
		local itemBuyout = state.quantity > 0 and Math.Ceil(state.totalPrice / state.quantity, LibTSMUI.IsCataPandaClassic() and 1 or COPPER_PER_SILVER)
...
function private.StateToPriceString(state)
	local itemBuyout = state.quantity > 0 and Math.Ceil(state.totalPrice / state.quantity, LibTSMUI.IsCataPandaClassic() and 1 or COPPER_PER_SILVER) or 0
