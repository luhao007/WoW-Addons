...
function AuctionBuyScan.__private:_ActionHandler(manager, state, action, ...)
	if action == "ACTION_START_SEARCH" then
		...
		if state.scanType == SCAN_TYPE.BROWSE then
			resolveSellers = LibTSMUI.IsVanillaClassic() or LibTSMUI.IsBCClassic()
	...
	elseif action == "ACTION_SET_SELECTED_AUCTION" then
		...
		if selection and selection:IsSubRow() then
			...
			state.selectionCanCancel = (not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic()) and self._isPlayerFunc(ownerStr, false)
	...
	elseif action == "ACTION_BUY_AUCTION_CONFIRMED" then
		...
		state.lastBuyQuantity = 0
		state.lastBuyIndex = nil
		local index = (LibTSMUI.IsVanillaClassic() or LibTSMUI.IsBCClassic()) and tremove(state.findResult, #state.findResult) or nil
		if (LibTSMUI.IsVanillaClassic() or LibTSMUI.IsBCClassic()) and not index then
		...
		if (LibTSMUI.IsVanillaClassic() or LibTSMUI.IsBCClassic()) and buyout ~= select(5, AuctionHouse.GetBrowseResult(index)) then
		...
		state.numBought = state.numBought + ((not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic()) and quantity or 1)
	...
	elseif action == "ACTION_BUYOUT_FUTURE_DONE" then
		...
		if result then
			...
			state.numConfirmed = min(state.numConfirmed + ((not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic()) and state.lastBuyQuantity or 1), state.numFound)
			...
			if state.lastBuyQuantity > 0 then
				state.numBought = state.numBought - ((not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic()) and state.lastBuyQuantity or 1)
				if LibTSMUI.IsVanillaClassic() or LibTSMUI.IsBCClassic() then
	...
	elseif action == "ACTION_REMOVE_BOUGHT_AUCTIONS" then
		...
		if selection and not selection:IsSubRow() then
			...
			local maxQuantity = state.searchContext:GetMaxCanBuy(itemString)
			if maxQuantity then
				if (LibTSMUI.IsVanillaClassic() or LibTSMUI.IsBCClassic()) and selection then
	...
	elseif action == "ACTION_BID_AUCTION_CONFIRMED" then
		...
		state.lastBuyQuantity = quantity
		state.lastBuyIndex = index
		state.numBid = state.numBid + ((not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic()) and quantity or 1)
	...
	elseif action == "ACTION_CANCEL_AUCTION" then
		assert((not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic()) and state.selectedAuction and state.selectedAuction:IsSubRow())
	...
	elseif action == "ACTION_POST_AUCTION" then
		...
		local undercut = ((not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic()) or self._isPlayerFunc(ownerStr, true)) and 0 or 1
		...
		state.auctionScrollTable:GetBaseElement():ShowDialogFrame(UIElements.New("ShoppingPostDialog", "dialog")
			:SetSize(326, (LibTSMUI.IsVanillaClassic() or LibTSMUI.IsBCClassic()) and 380 or 344)
			:AddAnchor("CENTER")
	...
	elseif action == "ACTION_POST_AUCTION_CONFIRMED" then
		...
		if not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic() then
			numStacks = 1
...
function private.GetConfirmationDialog(state, isBuy, alertThreshold)
	...
	local isCommodity = (not LibTSMUI.IsVanillaClassic() and not LibTSMUI.IsBCClassic()) and state.selectedAuction:IsCommodity()
