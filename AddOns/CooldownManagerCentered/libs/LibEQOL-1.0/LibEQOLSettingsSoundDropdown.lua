...
LibEQOLCMC_SoundDropdownMixin = CreateFromMixins(SettingsListElementMixin)
...
function LibEQOLCMC_SoundDropdownMixin:OnLoad()
...
function LibEQOLCMC_SoundDropdownMixin:Init(initializer)
...
function LibEQOLCMC_SoundDropdownMixin:GetSetting()
...
function LibEQOLCMC_SoundDropdownMixin:SetupLabel()
...
function LibEQOLCMC_SoundDropdownMixin:SetupPreviewButton()
    ...
    if type(anchor) == "table" then
        anchor = {
            anchor[1] or anchor.point or "LEFT",
            anchor[2],
            anchor[3] or (anchor[2] and "CENTER" or "LEFT"),
            (anchor[4] or anchor.x or anchor.offsetX or 0) + 3,
            anchor[5] or anchor.y or anchor.offsetY or 0,
        }
    end
...
function LibEQOLCMC_SoundDropdownMixin:SetupDropdown()
...
function LibEQOLCMC_SoundDropdownMixin:BuildDropdownMenu(rootDescription)
    ...
    for _, entry in ipairs(entries) do
        ...
        rootDescription:CreateRadio(label, function()
            return self:IsValueSelected(value)
        end, function()
            self:SetCurrentValue(value)
            self:UpdateDropdownText()
            if self.callback then
                self.callback(value, entry)
            end
        end, entry)
    end
end

function LibEQOLCMC_SoundDropdownMixin:GetSoundEntries()
    local list
    ...
    if list then
        return NormalizeOptions(list, self.data and self.data.order)
    end
...
function LibEQOLCMC_SoundDropdownMixin:IsValueSelected(value)
...
function LibEQOLCMC_SoundDropdownMixin:GetCurrentValue()
...
function LibEQOLCMC_SoundDropdownMixin:SetCurrentValue(value)
    local data = GetInitializerData(self)
    if data.setValue then
        ...
        local setting = self:GetSetting()
        if setting then
            setting:SetValue(value or "")
        end
    end
end

function LibEQOLCMC_SoundDropdownMixin:GetLabelForValue(value)
    if not value or value == "" then
        return nil
    end
    for _, entry in ipairs(self:GetSoundEntries()) do
        if entry.value == value then
            return entry.label or entry.value
        end
    end
    return value
end

function LibEQOLCMC_SoundDropdownMixin:UpdateDropdownText()
...
function LibEQOLCMC_SoundDropdownMixin:GetPlaybackChannel()
    ...
    if type(self.playbackChannel) == "string" and self.playbackChannel ~= "" then
        return self.playbackChannel
    end
end

function LibEQOLCMC_SoundDropdownMixin:ResolveSoundFile(value)
    if not value or value == "" then
        return
    end
    if self.soundResolver then
        return self.soundResolver(value)
    end
    if LSM then
...
function LibEQOLCMC_SoundDropdownMixin:PreviewSound()
...
function LibEQOLCMC_SoundDropdownMixin:RegisterSettingListener()
    local setting = self:GetSetting()
    if not (setting and self.cbrHandles) then
        return
    end
...
function LibEQOLCMC_SoundDropdownMixin:EvaluateState()
    SettingsListElementMixin.EvaluateState(self)
    local enabled = true
    if self.parentCheck then
        enabled = self.parentCheck() and true or false
    end
...
function LibEQOLCMC_SoundDropdownMixin:Release()
