...
LibEQOLCMC_ScrollDropdownMixin = CreateFromMixins(SettingsDropdownControlMixin)
...
function LibEQOLCMC_ScrollDropdownMixin:OnLoad()
    SettingsDropdownControlMixin.OnLoad(self)
end

function LibEQOLCMC_ScrollDropdownMixin:Init(initializer)
...
function LibEQOLCMC_ScrollDropdownMixin:GetSetting()
    if self.initializer and self.initializer.GetSetting then
        return self.initializer:GetSetting()
    end
    if self.data and self.data.setting then
        return self.data.setting
    end
    return nil
end

function LibEQOLCMC_ScrollDropdownMixin:GetOptions()
...
function LibEQOLCMC_ScrollDropdownMixin:RefreshDropdownText(value)
...
function LibEQOLCMC_ScrollDropdownMixin:SetValue(value)
...
function LibEQOLCMC_ScrollDropdownMixin:InitDropdown()
    local setting = self:GetSetting()
...
function LibEQOLCMC_ScrollDropdownMixin:SetupDropdownMenu(button, setting, optionsFunc, initTooltip)
