...
LibEQOLCMC_InputControlMixin = CreateFromMixins(SettingsControlMixin)
...
function LibEQOLCMC_InputControlMixin:ConfigureEditBox(editBox, multiline)
    if not editBox then
        return
    end
...
function LibEQOLCMC_InputControlMixin:OnLoad()
...
function LibEQOLCMC_InputControlMixin:Init(initializer)
...
function LibEQOLCMC_InputControlMixin:GetEditBox()
    return self.multiline and self.ScrollEditBox or self.Input
end

function LibEQOLCMC_InputControlMixin:UpdateScrollFrameWidth()
...
function LibEQOLCMC_InputControlMixin:ApplyLayout()
...
function LibEQOLCMC_InputControlMixin:HandleTextChanged(box, userInput)
...
function LibEQOLCMC_InputControlMixin:Revert(box)
    self._suppressInput = true
    box:SetText(self.displayValue or "")
    box:HighlightText()
    self._suppressInput = nil
...
function LibEQOLCMC_InputControlMixin:Commit(box)
...
function LibEQOLCMC_InputControlMixin:SetValue(value)
    self.currentValue = value
...
function LibEQOLCMC_InputControlMixin:OnSettingValueChanged(setting, value)
    SettingsControlMixin.OnSettingValueChanged(self, setting, value)
    self:SetValue(value)
end

function LibEQOLCMC_InputControlMixin:EvaluateState()
