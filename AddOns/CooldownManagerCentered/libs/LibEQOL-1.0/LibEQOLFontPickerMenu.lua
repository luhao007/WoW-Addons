...
local lib = ns.LibEQOL.FontPickerMenu or {}
ns.LibEQOL.FontPickerMenu = lib

-- Custom font picker popup modelled on LibEQOLSoundPickerMenu. Unlike a
-- MenuUtil submenu this picker owns all of its frames, so every row can be
-- rendered in its own font (SetFont is unrestricted here) and there is no
-- compositor/scrollbox interaction to fight.
    ...
    for _, option in ipairs(source or {}) do
        ...
        if value ~= nil and label and label ~= "" then
            entries[#entries + 1] = {
                ...
                fontPath = type(option) == "table" and option.fontPath or nil,
                data = option,
            }
        end
    end
    ...
    function picker:UpdateVisibleRows()
        ...
        for index = 1, VISIBLE_ROWS do
            ...
            if entry then
                ...
                -- Render the entry in its own font (preloads it and gives a
                -- live preview); reset to the default for the "Default font"
                -- row so recycled rows never keep a stale font.
                if entry.fontPath and entry.fontPath ~= "" then
                    pcall(row.label.SetFont, row.label, entry.fontPath, 12)
                else
...
function lib:CreateFontPickerMenu(parentDescription, data)
    assert(parentDescription and type(data) == "table", "parentDescription and data are required")
    local state = {}
    -- The entry point stays a static "Font" row rendered in the menu's default
    -- font; only the picker popup rows render in their own fonts.
    local buttonLabel = data.buttonLabel or "Font"
    local buttonDescription = parentDescription:CreateButton(buttonLabel, function()
        if state.owner then
    ...
    buttonDescription:AddInitializer(function(button, _, menu)
        state.owner = button
        state.menu = menu
        button.fontString:SetTextToFit(buttonLabel)
