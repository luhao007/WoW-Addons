...
LibEQOLCMC_ColorOverridesMixin = CreateFromMixins(SettingsListElementMixin)
...
function LibEQOLCMC_ColorOverridesMixin:OnLoad()
...
function LibEQOLCMC_ColorOverridesMixin:Init(initializer)
...
function LibEQOLCMC_ColorOverridesMixin:GetSpacing()
...
function LibEQOLCMC_ColorOverridesMixin:RefreshRows()
...
function LibEQOLCMC_ColorOverridesMixin:SetupRow(frame, entry)
    frame.data = entry
    if frame.Text then
...
function LibEQOLCMC_ColorOverridesMixin:ApplyTextColor(frame)
...
function LibEQOLCMC_ColorOverridesMixin:ResolveColor(key, useDefault)
    local r, g, b, a
    if useDefault then
...
function LibEQOLCMC_ColorOverridesMixin:ApplyColor(key, r, g, b, a)
...
function LibEQOLCMC_ColorOverridesMixin:RefreshRow(frame)
...
function LibEQOLCMC_ColorOverridesMixin:RefreshAll()
    for _, frame in ipairs(self.colorOverrideFrames or {}) do
        self:RefreshRow(frame)
    end
end

function LibEQOLCMC_ColorOverridesMixin:ResetToDefaults()
...
function LibEQOLCMC_ColorOverridesMixin:OpenColorPicker(frame)
    if not (self.setColor or self.setColorMixin) then
        return
    end
...
function LibEQOLCMC_ColorOverridesMixin:Release()
...
function LibEQOLCMC_ColorOverridesMixin:EvaluateState()
