...
LibEQOLCMC_SortableListMixin = CreateFromMixins(SettingsListElementMixin)
...
function LibEQOLCMC_SortableListMixin:OnLoad()
...
function LibEQOLCMC_SortableListMixin:Init(initializer)
...
function LibEQOLCMC_SortableListMixin:ReadItems()
...
function LibEQOLCMC_SortableListMixin:Commit(action, item, oldIndex, newIndex)
...
function LibEQOLCMC_SortableListMixin:MoveItem(index, delta)
...
function LibEQOLCMC_SortableListMixin:MoveItemToIndex(fromIndex, toIndex)
...
function LibEQOLCMC_SortableListMixin:DeleteItem(index)
...
function LibEQOLCMC_SortableListMixin:AddItem(item)
    if item == nil then
        return
    end
...
function LibEQOLCMC_SortableListMixin:ResolveAddOptions()
    local data = self.data or {}
    local options
...
function LibEQOLCMC_SortableListMixin:OpenAddMenu()
    local data = self.data or {}
...
function LibEQOLCMC_SortableListMixin:AddMenuOption(option, text)
...
function LibEQOLCMC_SortableListMixin:ShowDropHighlight(frame, show)
...
function LibEQOLCMC_SortableListMixin:ClearDropHighlights()
    for _, frame in ipairs(self.rowFrames or {}) do
        self:ShowDropHighlight(frame, false)
    end
end

function LibEQOLCMC_SortableListMixin:GetDropIndex()
...
function LibEQOLCMC_SortableListMixin:UpdateDragGhostPosition()
    ...
    ghost:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT", (x / scale) + (self.dragGhostOffsetX or 0), (y / scale) + (self.dragGhostOffsetY or 0))
end

function LibEQOLCMC_SortableListMixin:EnsureDragGhost()
...
function LibEQOLCMC_SortableListMixin:ShowDragGhost(text)
...
function LibEQOLCMC_SortableListMixin:HideDragGhost()
    if self.dragGhost then
        self.dragGhost:Hide()
    end
end

function LibEQOLCMC_SortableListMixin:SetupRow(frame, item, index)
...
function LibEQOLCMC_SortableListMixin:RefreshRows()
    if self.data then
...
function LibEQOLCMC_SortableListMixin:Release()
...
function LibEQOLCMC_SortableListMixin:EvaluateState()
