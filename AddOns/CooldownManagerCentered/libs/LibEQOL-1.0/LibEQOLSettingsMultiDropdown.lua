local _, ns = ...
local EXPECTED_MINOR = 24000003
local lib = ns.LibEQOL.SettingsMode
if lib.MINOR and lib.MINOR > EXPECTED_MINOR then
    return
end

LibEQOLCMC_MultiDropdownMixin = CreateFromMixins(SettingsDropdownControlMixin)
...
function LibEQOLCMC_MultiDropdownMixin:OnLoad()
...
function LibEQOLCMC_MultiDropdownMixin:GetSetting()
...
function LibEQOLCMC_MultiDropdownMixin:CloneOption(option)
    local cloned = {}
    ...
    if cloned.value == nil and cloned.label then
        cloned.value = cloned.label
    end
...
function LibEQOLCMC_MultiDropdownMixin:SetOptions(list)
...
function LibEQOLCMC_MultiDropdownMixin:GetOptions()
...
function LibEQOLCMC_MultiDropdownMixin:Init(initializer)
...
function LibEQOLCMC_MultiDropdownMixin:SetValue(value)
...
function LibEQOLCMC_MultiDropdownMixin:GetLegacySelectionTable()
    ...
    if self.subvar then
        if type(container[self.subvar]) ~= "table" then
            container[self.subvar] = {}
        end
...
function LibEQOLCMC_MultiDropdownMixin:RefreshSelectionCache()
    local selection

    if self.getSelectionFunc then
        local ok, result = pcall(self.getSelectionFunc)
        if ok then
            selection = NormalizeSelection(result)
        end
    end
...
function LibEQOLCMC_MultiDropdownMixin:GetSelectionMap()
    return self.selectionCache or self:RefreshSelectionCache()
end

function LibEQOLCMC_MultiDropdownMixin:GetSelectionMapSnapshot()
    return CopySelection(self:GetSelectionMap())
end

function LibEQOLCMC_MultiDropdownMixin:IsSelected(key, option)
...
function LibEQOLCMC_MultiDropdownMixin:ApplyLegacySelection(selection)
...
function LibEQOLCMC_MultiDropdownMixin:SetSelected(key, shouldSelect, option)
...
function LibEQOLCMC_MultiDropdownMixin:SyncSetting(selection)
    if self._eqolSuppressSync then
        return
    end
...
function LibEQOLCMC_MultiDropdownMixin:ToggleOption(key, option)
...
function LibEQOLCMC_MultiDropdownMixin:SerializeSelection(tbl)
...
function LibEQOLCMC_MultiDropdownMixin:RefreshSummary()
...
function LibEQOLCMC_MultiDropdownMixin:ApplyDefaultSelection()
...
function LibEQOLCMC_MultiDropdownMixin:EnsureDefaultCallbacks()
...
function LibEQOLCMC_MultiDropdownMixin:EnsureSummaryAnchors()
...
function LibEQOLCMC_MultiDropdownMixin:GetSummaryWidthLimit()
...
function LibEQOLCMC_MultiDropdownMixin:GetSummaryMeasureFontString()
...
function LibEQOLCMC_MultiDropdownMixin:WouldExceedSummaryWidth(text, widthLimit)
...
function LibEQOLCMC_MultiDropdownMixin:FormatSummaryText(texts)
    if #texts == 0 then
        return "–"
    end
...
function LibEQOLCMC_MultiDropdownMixin:InitDropdown()
    local setting = self:GetSetting()
...
function LibEQOLCMC_MultiDropdownMixin:SetupDropdownMenu(button, setting, optionsFunc, initTooltip)
