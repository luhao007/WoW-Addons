...
local baseLocale = {
    ...
    ["Coordinates can be moved from the default location, this setting allows you to control that offset"] = "可以从默认位置滑动坐标，以适应其他插件。此设置可以控制该偏移量",
    ...
    ["Icon Control"] = "图标控制",
    ["If you have changed the waypoint display settings (minimap, world), this will reset all waypoints to the current options."] = "如果您已更改路径点显示设置（小地图，世界），这将重新设置所有路径点到当前的选项。",
    ...
    ["Resets the position of the waypoint arrow if it has been dragged off screen"] = "重置路径点箭头的位置，如果它被拖出屏幕",
    ...
    ["TomTom can display a tooltip containing information about waypoints, when they are moused over.  This setting toggles that functionality"] = "当鼠标指向路径点时，TomTom 可以显示包含本路径点信息的鼠标提示。这个设置可以切换本功能",
    ...
    ["|cffffff78/way <x> <y> [desc]|r - Adds a waypoint at x,y with description desc"] = "|cffffff78/way <x> <y> [注释]|r - 于 X，Y 坐标处添加一路径点并添加注释描述",
