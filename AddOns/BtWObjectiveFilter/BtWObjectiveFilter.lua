...
hooksecurefunc("ObjectiveTracker_ReorderModules", function ()
    ObjectiveTrackerFrame.HeaderMenu.Title:SetShown(ObjectiveTrackerFrame.collapsed)
    if ObjectiveTrackerFrame.HeaderMenu:GetPoint(1) == nil then
        ObjectiveTrackerFrame.HeaderMenu.Title:Show()
        ObjectiveTrackerFrame.HeaderMenu:SetPoint("TOPRIGHT", ObjectiveTrackerFrame, "TOPRIGHT", -10, -12)
    end    
end)
ObjectiveTrackerFrame.HeaderMenu:HookScript("OnHide", function (self)
    self:Show()
end)
...
local function eventHandler(self, event, ...)
    if event == "PLAYER_ENTERING_WORLD" then
        if initCounter == 10 then
            return
        end

        if ObjectiveTrackerFrame and ObjectiveTrackerFrame.initialized then
