...
function LBIS:CreateSettings()
	...
	-- Reset filter settings that don't match a current localized option.
	-- Older saves could store English literals that lock out items after a language change.
	local validAll = LBIS.L["All"]
	local validSourceTypes = { [LBIS.L["All"]]=true, [LBIS.L["Drop"]]=true, [LBIS.L["Raid"]]=true, [LBIS.L["Crafted"]]=true }
	if not validSourceTypes[LBISSettings.SelectedSourceType] then
		LBISSettings.SelectedSourceType = validAll
	end
	-- Slot/Zone: reset only if stuck on the English "All" while the locale uses something else.
	if LBISSettings.SelectedSlot == "All" and validAll ~= "All" then
		LBISSettings.SelectedSlot = validAll
	end
	if LBISSettings.SelectedZone == "All" and validAll ~= "All" then
		LBISSettings.SelectedZone = validAll
	end

	LibStub("AceConfig-3.0"):RegisterOptionsTable("Loon Best In Slot", CreateOptions(), nil)
	local optionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("Loon Best In Slot")
	if InterfaceOptionsFramePanelContainer then
		optionsFrame:SetParent(InterfaceOptionsFramePanelContainer)
	end
	-- Store category ID for Midnight Settings.OpenToCategory
	if optionsFrame and optionsFrame.categoryID then
		LBIS.OptionsCategoryID = optionsFrame.categoryID
	elseif Settings and Settings.GetCategoryCount then
		-- Search for our category
		for i = 1, Settings.GetCategoryCount() do
			local info = Settings.GetCategoryInfo(i)
			if info and info == "Loon Best In Slot" then
				LBIS.OptionsCategoryID = i
				break
			end
		end
	end
