...
local function createItemRow(f, specEnchant, specEnchantSource)
    ...
    -- Use GetItemInfo for item enchants, GetSpellInfo for spell enchants
    local infoFunc = (specEnchantSource.IsSpell == "False") and LBIS.GetItemInfo or LBIS.GetSpellInfo
    local isSpell = (specEnchantSource.IsSpell ~= "False")
    infoFunc(LBIS, specEnchant.Id, function(item)
    ...
    local _, count = string.gsub(specEnchantSource.Source or "", "/", "")
    if count > 1 then
        count = count - 1;
    else 
        count = 0;
...
function LBIS.EnchantList:UpdateItems()
    ...
    LBIS.BrowserWindow:UpdateItemsForSpec(function(point)
        ...
        local specEnchants = LBIS.EnchantsBySpecAndId[(LBIS:ResolveSelectedSpec())];
