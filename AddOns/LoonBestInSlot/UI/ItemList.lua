...
local function itemSortFunction(table, k1, k2)
    ...
    -- unknown slots sort to the end instead of erroring
    local order1 = itemSlotOrder[item1.Slot] or 999
    local order2 = itemSlotOrder[item2.Slot] or 999
...
local function IsInPhase(specItem, specItemSource)
    -- Retail Midnight: no phase filtering, always show all items
    return true;
end
    ...
    if LBISSettings.HideObsolete then
        ...
        if itemId1 ~= nil and tonumber(itemId1) > 0 then
            if LBIS.ItemsByIdAndSpec[tonumber(itemId1)] ~= nil then
                local cachedItem1 = LBIS.ItemsByIdAndSpec[tonumber(itemId1)][(LBIS:ResolveSelectedSpec())];
        ...
        if itemId2 ~= nil and tonumber(itemId2) > 0 then    
            if LBIS.ItemsByIdAndSpec[tonumber(itemId2)] ~= nil then    
                local cachedItem2 = LBIS.ItemsByIdAndSpec[tonumber(itemId2)][(LBIS:ResolveSelectedSpec())];
...
function LBIS.ItemList:UpdateItems()
    ...
    LBIS.BrowserWindow:UpdateItemsForSpec(function(point)
        ...
        local specItems = LBIS.ItemsBySpecAndId[(LBIS:ResolveSelectedSpec())];
        ...
        for itemId, specItem in LBIS:spairs(specItems, itemSortFunction) do
            local specItemSource = LBIS.ItemSources and LBIS.ItemSources[specItem.Id];
