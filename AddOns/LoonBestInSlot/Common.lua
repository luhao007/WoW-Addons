...
function LBIS:PreCacheItems()
    ...
    for itemId, _ in pairs(LBIS.ItemSources or {}) do
...
-- Upgrade Track System (Midnight Season 1)
-- Track bonus IDs from KeystoneLoot (3 groups × 6 entries each)
-- Colors: Green=Uncommon, Blue=Rare, Purple=Epic, Orange=Legendary
LBIS.UpgradeTracks = {
    -- Champion track (4 green + 2 blue)
    { label = "|cff1eff00246|r (+0)",         bonusId = 12785, ilvl = 246, group = "Champion" },
    { label = "|cff1eff00250|r (+2 +3)",      bonusId = 12786, ilvl = 250, group = "Champion" },
    { label = "|cff1eff00253|r (+4)",          bonusId = 12787, ilvl = 253, group = "Champion" },
    { label = "|cff1eff00256|r (+5)",          bonusId = 12788, ilvl = 256, group = "Champion" },
    { label = "|cff0070dd259|r (Upgrade)",     bonusId = 12789, ilvl = 259, group = "Champion" },
    { label = "|cff0070dd263|r (Upgrade)",     bonusId = 12790, ilvl = 263, group = "Champion" },
    -- Hero track (4 blue + 2 purple)
    { label = "|cff0070dd259|r (+6 +7)",       bonusId = 12793, ilvl = 259, group = "Hero" },
    { label = "|cff0070dd263|r (+8 +9)",       bonusId = 12794, ilvl = 263, group = "Hero" },
    { label = "|cff0070dd266|r (+10)",         bonusId = 12795, ilvl = 266, group = "Hero" },
    { label = "|cff0070dd269|r (Upgrade)",     bonusId = 12796, ilvl = 269, group = "Hero" },
    { label = "|cffa335ee272|r (Upgrade)",     bonusId = 12797, ilvl = 272, group = "Hero" },
    { label = "|cffa335ee276|r (Upgrade)",     bonusId = 12798, ilvl = 276, group = "Hero" },
    -- Great Vault / Myth track (4 purple + 2 orange)
    { label = "|cffa335ee272|r (+10)",         bonusId = 12801, ilvl = 272, group = "Great Vault" },
    { label = "|cffa335ee276|r (Upgrade)",     bonusId = 12802, ilvl = 276, group = "Great Vault" },
    { label = "|cffa335ee279|r (Upgrade)",     bonusId = 12803, ilvl = 279, group = "Great Vault" },
    { label = "|cffa335ee282|r (Upgrade)",     bonusId = 12804, ilvl = 282, group = "Great Vault" },
    { label = "|cffff8000285|r (Upgrade)",     bonusId = 12805, ilvl = 285, group = "Great Vault" },
    { label = "|cffff8000289|r (Max)",         bonusId = 12806, ilvl = 289, group = "Great Vault" },
}

-- All track bonus IDs (used to identify which bonus to swap)
local ALL_TRACK_BONUS_IDS = {}
for _, track in ipairs(LBIS.UpgradeTracks) do
    ALL_TRACK_BONUS_IDS[tostring(track.bonusId)] = true
end
-- Also include raid-specific ones
for _, bid in ipairs({12777,12778,12779,12780,12781,12782,12783,12784,12785,12786,12787,12788,12789,12790,12791,12792,12793,12794,12795,12796,12797,12798,12799,12800,12801,12802,12803,12804,12805,12806}) do
    ALL_TRACK_BONUS_IDS[tostring(bid)] = true
end

-- Current selected track (default: Myth 289)
LBIS.SelectedTrackBonusId = 12806

-- Build an item string with bonus IDs, swapping track bonus to match selection
local function BuildBISItemString(itemId)
    local bonusString = LBIS.ItemBonuses and LBIS.ItemBonuses[itemId]
    if bonusString and bonusString ~= "" then
        -- Split bonus IDs and swap any track bonus with the selected one
        local parts = {}
        local hasTrackBonus = false
        for b in bonusString:gmatch("[^:]+") do
            if ALL_TRACK_BONUS_IDS[b] then
                table.insert(parts, tostring(LBIS.SelectedTrackBonusId))
                hasTrackBonus = true
            else
                table.insert(parts, b)
            end
        end
        -- If no track bonus found, add the selected one
        if not hasTrackBonus then
            table.insert(parts, tostring(LBIS.SelectedTrackBonusId))
        end
        local newBonus = table.concat(parts, ":")
        return "item:" .. itemId .. "::::::::::::" .. #parts .. ":" .. newBonus
    end
    return "item:" .. itemId
end
...
function LBIS:GetItemInfo(itemId, returnFunc)
    ...
    if cachedItem then
        ...
        -- Build item string with bonus IDs for correct ilvl
        local itemString = BuildBISItemString(itemId)

        -- Use GetItemInfo with the bonus-ID item string to get a resolved link
        local itemName, itemLink, itemQuality, _, _, _, _, _, _, itemIcon = C_Item.GetItemInfo(itemString)

        if itemName then
            -- Item is already cached by the client
            local _, itemType, subType, itemSlot, _, classId = C_Item.GetItemInfoInstant(itemId);
            ...
            if LBIS.ItemSources and LBIS.ItemSources[itemId] ~= nil then
                LBISServerSettings.ItemCache[itemId] = newItem;
            end

            returnFunc(newItem);
        else
            -- Item not cached yet, use CreateFromItemID to trigger load
            local itemCache = Item:CreateFromItemID(itemId)

            itemCache:ContinueOnItemLoad(function()
                local _, itemType, subType, itemSlot, _, classId = C_Item.GetItemInfoInstant(itemId);

                -- Now try again with bonus IDs - item data should be available
                local name, link, quality, _, _, _, _, _, _, icon = C_Item.GetItemInfo(itemString)
                if not name then
                    -- Fallback to base item
                    name = itemCache:GetItemName()
                    link = itemCache:GetItemLink()
                    quality = itemCache:GetItemQuality()
                    icon = itemCache:GetItemIcon()
                end
...
-- Detect retail dropdown support
LBIS_HAS_RETAIL_DROPDOWN = false
do
    local ok = pcall(function()
        local test = CreateFrame("DropdownButton", "LBISDropdownTest", UIParent, "WowStyle1DropdownTemplate")
        test:Hide()
        test:SetParent(nil)
        LBIS_HAS_RETAIL_DROPDOWN = true
    end)
end
...
function LBIS:CreateDropdown(opts, width)
    ...
    if LBIS_HAS_RETAIL_DROPDOWN then
        -- Modern retail dropdown
        local dropdown = CreateFrame("DropdownButton", '$parent_' .. opts['name'] .. '_dropdown', opts['parent'], "WowStyle1DropdownTemplate")

        local selectedVal = default_val
        ...
        dropdown:SetupMenu(function(dd, rootDescription)
            for _, val in ipairs(menu_items) do
                rootDescription:CreateRadio(val,
                    function(data) return selectedVal == data end,
                    function(data)
                        selectedVal = data
                        change_func(dropdown, data)
                    end,
                    val
                )
            end
...
function LBIS:SetTooltipOnButton(b, item, isSpell)
    ...
    -- Dynamically build item string on each tooltip show (respects track selection changes)
    local function GetCurrentItemString()
        return BuildBISItemString(b.ItemId)
    end

    b.ShowTooltip = function ()
        GameTooltip:SetOwner(b, "ANCHOR_RIGHT");
        GameTooltip:SetHyperlink(GetCurrentItemString());
        GameTooltip:Show();
    end
...
local function printItemSource(itemId, specItemSource, dl, printNumber, printLocation)
    ...
    local function printSourceText(sourceText, sourceNumberText, sourceLocationText, firstRow)
        ...
        for index, source in pairs(sourceSplit) do		
            ...
            text = text..LBIS:LocalizeSource(strtrim(source));
        ...
        if sourceLocationText ~= nil and sourceLocationText ~= "" and printLocation then
            text = text.." - "..LBIS:LocalizeSource(sourceLocationText);
        end
    end
...
local function createSourceTypeText(specItemSource)
    ...
    local function getSourceColor(sourceType)
        if sourceType == LBIS.L["Profession"] or sourceType == LBIS.L["Crafted"] then
            return "|cFF33ADFF";
        elseif sourceType == LBIS.L["Raid"] then
            return "|cFFFC6A03";
