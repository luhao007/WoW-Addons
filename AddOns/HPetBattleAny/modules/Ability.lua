...
AbilityModules.GetAbilityInfo = function(petOwner, petIndex, i, minmod)
	if not C_PetBattles.IsPlayerNPC() and petOwner ~= 1 then
		if not minmod then
...
local function PetBattleAbilityButton_OnEnterhook(self)
	...
	if (self:GetEffectiveAlpha() > 0 and SelfGetAbilityInfo(petOwner, petIndex, self:GetID()) and C_PetBattles.IsPlayerNPC()) then
...
function CreateAbilityList(targetname, rpoints)
	...
	function frame:GetPetXY()
		if PetBattleFrame and PetBattleFrame[self.unit] then
		 ...
		 ---C_PetBattles.GetActivePet(PetBattleFrame[self.unit].petOwner)
		else
			return 2, 1
		end
	end
	...
	function frame:Update(event)
		...
		if (C_PetBattles.IsPlayerNPC() or petOwner == 1) then
			for i = 4, 6 do
	...
	function frame:Ref()
		if not HPetSaves.AbPoint then
			HPetSaves.AbPoint = {}
		end
		if not HPetSaves.AbPoint[targetname] then
			HPetSaves.AbPoint[targetname] = self.defaultPoint or {}
		end
		...
		self:ClearAllPoints()
		self:SetPoint(HPetSaves.AbPoint[targetname][1] or "BOTTOM", _G[HPetSaves.AbPoint[targetname][2]] or nil, HPetSaves.AbPoint[targetname][3] or "BOTTOM", HPetSaves.AbPoint[targetname][4] or "000", HPetSaves.AbPoint[targetname][5] or "540")
		self:TextToggle(HPetSaves.ShowAbilitysName)

		if (targetname:find("Enemy") and HPetSaves.EnemyAbility and (targetname:find("Active") or HPetSaves.OtherAbility)) or (targetname:find("Ally") and HPetSaves.AllyAbility and HPetSaves.OtherAbility) then
			self:Show()
		else
			self:Hide()
		end
	...
	function frame:InitButton()
		for i = 1, 6 do
			...
			Button:SetScript(
				"OnDragStart",
				function(self)
					if HPetSaves and not HPetSaves.LockAbilitys then
			...
			Button:SetScript(
				...
				function(self)
					...
					if HPetSaves then
						...
						if HPetSaves.AbPoint[targetname][2] then
							HPetSaves.AbPoint[targetname][2] = HPetSaves.AbPoint[targetname][2]:GetName()
						end
					end
				end
			)
...
HPetBattleAny.FirstAbilitysLoad = function()
	...
	if HPetSaves.OtherAbility then
		...
		HPetBattleAny.AllyAbilitys["Ally2"] = HPetBattleAny.AllyAbilitys["Ally2"] or CreateAbilityList("Ally2", {"RIGHT", HPetBattleAny.AllyAbilitys["ActiveAlly"]:GetName(), "LEFT", 400, 0})
		 ---200,340)
		HPetBattleAny.AllyAbilitys["Ally3"] = HPetBattleAny.AllyAbilitys["Ally3"] or CreateAbilityList("Ally3", {"LEFT", HPetBattleAny.AllyAbilitys["ActiveAlly"]:GetName(), "RIGHT", -400, 0})
	 --200,340)
	end
	--~ 	end
	...
	if CopPtable(enemytab, wtable) then
		if CP.GetAbilityInfo(1, CP.GetActivePet(1), useA) and tonumber(wtable[1]) == CP.GetAbilityInfo(1, CP.GetActivePet(1), useA) then
			if CopPtable(allytab, wtable) then
				if ttable[1] == "H" then
					local tar = ttable[2]
					if tar == PET_BATTLE_COMBAT_LOG_YOUR_LOWER or tar == PET_BATTLE_COMBAT_LOG_YOUR then
						--~ 					print("AAH1")
						return 0
					end

					if tar == PET_BATTLE_COMBAT_LOG_ENEMY_LOWER or tar == PET_BATTLE_COMBAT_LOG_ENEMY then
						--~ 					print("AAH2")
						return 1
					end
				end
				...
				if ttable[1] == "B" then
					...
					if tar == PET_BATTLE_COMBAT_LOG_ENEMY_LOWER or tar == PET_BATTLE_COMBAT_LOG_ENEMY then
						--~ 					print("BAH2")
						if not tempPetBuffs[abilityID] then
							tempPetBuffs[abilityID] = {}
						end
						if not tempPetBuffs[abilityID].id then
							tempPetBuffs[abilityID].id = id
						end
...
f:SetScript(
	...
	function(self, event, msg, ...)
		if C_PetBattles.IsPlayerNPC() then
			return
		end
