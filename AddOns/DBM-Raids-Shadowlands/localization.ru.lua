...
L:SetOptionLocalization({
	...
	ShowTimeNotStacks		= "Показывать оставшееся время $spell:334755 на инфофрейме вместо количества стаков"
})
...
L:SetOptionLocalization({
	timerContainers2			= "Показывать таймер, который будет отображать прогресс заполнения контейнера и время, оставшееся до заполнения"
})
...
L:SetMiscLocalization({
	DBMConfigMsg		= "Настройки иконок установлены ​​на %s для соответствия конфигурации лидера рейда."
})
...
L:SetOptionLocalization({
	warnRemnant	= "Объявлять персональные стаки дебаффов Фрагмента"
})

L:SetMiscLocalization({
	Remnant			= "Фрагмент",
	PortalRP		= "Оцепите все вокруг! Защищайте территорию!"
})
...
L:SetMiscLocalization({
	...
	PrePull			= "Глупцы. Вы напрасно сражаетесь."
})
...
L:SetMiscLocalization({
	WallRP	= "Как удачно, что мы снова встретились за миг до окончательной победы Смерти."
})
...
L:SetOptionLocalization({
	RitualistIconSetting	= "Установить режим настройки меток Ритуалистов. Используется предпочтение лидера рейда, если он использует DBM.",
...
L:SetOptionLocalization({
	timerComboCD		= "Отсчёт времени (с подсчетом) до перезарядки танкового комбо"
})
...
L:SetOptionLocalization({
	PairingBehavior		= "Установить режим мода для $spell:361989. Используется предпочтение лидера рейда, если он использует DBM",
...
L:SetMiscLocalization({
	PrePull				= "Дело сделано. Последний чертог Гробницы распахнул свои двери."
})
...
L:SetOptionLocalization({
	timerPits			= "Отсчет времени до открытия лунок на полу, в которые можно упасть",
	...
	warnDispel			= "Показать предупреждение о том, когда нужно рассеять $spell:363772 на мифической сложности, основываясь на стратегии гильдии Эхо",
	timerHealAzeroth	= "Отсчет времени до того момента, когда Вам нужно исцелить Азерот (через механику боя) на мифической сложности, основываясь на стратегии гильдии Эхо",
	timerDispels		= "Отсчет времени до того момента, когда нужно рассеять 'Смертный приговор' на мифической сложности, основываясь на стратегии гильдии Эхо"
})
