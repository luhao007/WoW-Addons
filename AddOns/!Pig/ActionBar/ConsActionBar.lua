...
------------
function ActionBarfun.addOptions_CABar()
	...
	StaticPopupDialogs["CHONGZHI_CONSACTIONBAR"] = {
		...
		button2 = NO,
		OnAccept = function()
			PIGA["CABar"] = CopyTable(addonTable.Default["CABar"])
			PIG_OptionsUI.RLUI:Show()
		end,
		timeout = 0,
		whileDead = true,
	...
	CABarF:SetScript("OnShow", function (self)
		CABarF.Open:SetChecked(PIGA["CABar"]["Open"])
		if not CABarF.SetF then
			...
			CABarF.SetF.Lock:SetChecked(PIGA["CABar"]["Lock"])
			CABarF.SetF.suofang:PIGSetValue(PIGA["CABar"]["Scale"])
			CABarF.SetF.Pailie:PIGDownMenu_SetText(pailieName[PIGA["CABar"]["Pailie"]])
			...
			CABarF.ModF.Cons:SetChecked(PIGA["CABar"]["Cons"])
			CABarF.ModF.Quest:SetChecked(PIGA["CABar"]["Quest"])
		end
		CABarF.SetF:SetShown(PIGA["CABar"]["Open"])
		CABarF.ModF:SetShown(PIGA["CABar"]["Open"])
	end)
...
function ActionBarfun.CABar()
	...
	Actionbar:HookScript("OnEvent", function(self,event,arg1,arg2)
			...
			for slot=1,NumSlots do
				...
				if itemID and itemID~=6948 then
					...
					if usable or lootable then
						local _, itemType, itemSubType, itemEquipLoc, icon, classID, subClassID=PIGGetItemInfoInstant(itemID)
