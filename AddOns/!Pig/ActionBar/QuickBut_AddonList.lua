...
local GnUI,Icon,GnName,FrameLevel="PIG_AddonListUI","Mobile-BonusIcon",ADDONS..CHAT_MODERATE,55
Create.PIGaddQuickBut(6,{
	Open=function()
		return PIGA["QuickBut"]["Open"] and PIGA["QuickBut"]["AddonList"]
	end,
	Icon=Icon,
	ShowGnUI=GnUI,
	FrameLevel=55,
	fun=function(QkBut,QuickButUI)
		...
		local function PIG_loadAddon_(id)
			...
			if allcharacter then
				...
				DisableAllAddOns(PlayerInfo.Name)
			end			
			for k,v in pairs(addondata[2]) do
				if v then
					if allcharacter then
						...
						EnableAddOn(k,PlayerInfo.Name)
					end
				else
					if allcharacter then
						DisableAddOn(k)
					else
						DisableAddOn(k,PlayerInfo.Name)
					end
				end
			end
			ReloadUI();
		...
		--展开页
		local Height,anniushu,NumTexCoord  = QuickButUI.butWWW, 10, Data.EquipmentPIG["NumTexCoord"]
		...
		AdminF.List_C.Savebut.F.SaveBut:HookScript("OnClick", function (self)
			...
			for id=1,GetNumAddOns() do
				...
				local loadablex = PIGGetAddOnEnableState(id, PlayerInfo.Name)
				if loadablex>0 then
		        ...
		        if NewData[dataIndex] then
		        	if not AdminF.List_C.butList[i] then
						...
						hangL.Check:HookScript("OnClick", function(self)
						    ...
						    if AdminF.SelectLID==0 then
							    local currentState = PIGGetAddOnEnableState(index, PlayerInfo.Name)
					...
					if AdminF.SelectLID == 0 then
					    isEnabled = (PIGGetAddOnEnableState(index, PlayerInfo.Name) > 0)
					else
