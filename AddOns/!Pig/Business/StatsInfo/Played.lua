...
function BusinessInfo.Player(StatsUI)
	...
	local hang_Height,hang_NUM,UIWWW  = 22.6, 20,fujiF:GetWidth();
	local maxwwww=500
	local function add_uifun()
		...
		fujiF.Player.alltimeTXT = PIGFontString(fujiF.Player,{"TOPLEFT", fujiF.Player, "TOPLEFT", 10, -2})
		fujiF.Player.alltimeTXT:SetTextColor(0, 1, 0, 1);
		fujiF.Player.lineTOP = PIGLine(fujiF.Player,"TOP",-22,nil,nil,{0.3,0.3,0.3,0.8})
	...
	function fujiF.Update_List()
		...
		add_uifun()
		for _,hang in pairs(fujiF.Player.ButList) do
			hang:Hide()
	    ...
	    for id = 1, hang_NUM do
			...
			if cdmulu[dangqian] then
				if not fujiF.Player.ButList[id] then
					local hang = CreateFrame("Frame", nil, fujiF.Player);
					fujiF.Player.ButList[id]=hang
					hang:SetSize(fujiF.Player:GetWidth()-18,hang_Height);
					if id==1 then
						hang:SetPoint("TOPLEFT", fujiF.Player.Scroll, "TOPLEFT", 0, -1);
					else
						hang:SetPoint("TOPLEFT", fujiF.Player.ButList[id-1], "BOTTOMLEFT", 0, -2);
					end
					if id~=hang_NUM then
				...
				hang.nameDQ:SetShown(cdmulu[dangqian][5])
				hang.GetTime:SetShown(cdmulu[dangqian][5])
