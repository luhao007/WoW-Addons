...
--------------------
function GeneralFun.Interaction_RightPlus()
	if not PIGA["Interaction"]["RightPlus"] then return end
	if GeneralFun.RightPlusOpen then return end
	GeneralFun.RightPlusOpen=true
	if Menu and Menu.ModifyMenu then
		local function add_pigmenu(rootDescription, contextData,menulist)
		   	-- 在菜单末尾加入新按钮.
		   	local wanjiaName = contextData.name
			if contextData.server and PlayerInfo.Realm~=contextData.server then
		...
		PigRightF:Hide();
		GeneralFun.PigRightF=PigRightF
		if NDui then
		...
		local function ClickGongNeng(menuName,listName)
			...
			if serverX and PlayerInfo.Realm~=serverX then
