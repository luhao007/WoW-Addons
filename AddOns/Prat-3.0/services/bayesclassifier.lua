...
local function wordinfoset(db, word, record)
	-- "-" forces quote in SavedVariables in Warhammer Online and left as it is for WoW - <Mera>
...
local function verify_cache(db, wordstream, is_spam)
	...
	for _, word in ipairs(wordstream) do
		...
		if record ~= nil then
			if is_spam then
				if record.spamcount == 0 then
					-- the phrase has not been trained or slightly changed - <Mera>
					err = true
					break
				end
...
function private.GetClassifier(db)
	for k, v in pairs(defaults) do
		db[k] = db[k] or v
	end
	return {
		getprob = function(...)
			return chi2_spamprob(db, ...)
		end,
		learn = function(...)
			return add_msg(db, ...)
		end,
		unlearn = function(...)
			return remove_msg(db, ...)
		end,
	}
end
