...
RSVignettePinPOIButtonMixin = CreateFromMixins(VignettePinPOIButtonMixin);

RSVignettePinPOIButtonMixin.SetPassThroughButtons = function() end

function RSVignettePinPOIButtonMixin:OnAcquired(vignetteGUID, vignetteInfo, frameIndex)
	-- ORIGINAL CODE
...
function RSVignettePinPOIButtonMixin:OnMouseEnter()
	if (self.POI and RSConfigDB.IsShowingTooltipsOnIngameIcons()) then
		RSTooltip.ShowSimpleTooltip(self)
	else
		self.tooltip = nil
		VignettePinPOIButtonMixin.OnMouseEnter(self)
	end
end

function RSVignettePinPOIButtonMixin:OnMouseLeave()
	if (self.POI and RSConfigDB.IsShowingTooltipsOnIngameIcons()) then
		if (RSTooltip.HideTooltip(self.tooltip)) then
			self.tooltip = nil
		end
	else
		VignettePinPOIButtonMixin.OnMouseLeave(self)
	end
end

function RSVignettePinPOIButtonMixin:OnMouseDown(button)
	if (self.POI and RSConfigDB.IsShowingTooltipsOnIngameIcons()) then				
