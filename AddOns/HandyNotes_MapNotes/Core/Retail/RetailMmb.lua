...
function ns.GetMiniMapButtonIcon()
  if ns.Addon and ns.Addon.db and ns.Addon.db.profile and ns.Addon.db.profile.activate and ns.Addon.db.profile.activate.HideMapNote then
    return iconLink .. "Exit"
  end

  return iconLink .. "MNL4"
end

function ns.UpdateMiniMapButtonIcon()
  local iconPath = ns.GetMiniMapButtonIcon()

  if ns.miniButton then
    ns.miniButton.icon = iconPath
  end

  local button = _G["LibDBIcon10_MNMiniMapButton"]
  if button then
    if button.icon then
  ...
  if MNMMBIcon and MapNotesMiniButton and MapNotesMiniButton.db then
  ...
  OnClick = function(self, button)
    ...
    if not ns.Addon.db.profile.activate.HideMapNote then
      ...
      if button == "MiddleButton" and not IsShiftKeyDown() then
        ...
        if ns.UpdateMiniMapButtonIcon then ns.UpdateMiniMapButtonIcon() end
        if ns.WorldMapButton and ns.WorldMapButton.UpdateIconOverlay then ns.WorldMapButton:UpdateIconOverlay() end
